    ...
    stages {
        ...
        stage('Vulnerability Image Scanner') {
            steps {
                ...
                trivy image --format template --template "@html.tpl" -o /var/www/html/trivy/pipeline${BUILD_NUMBER}/reportimagesecretpython.html --exit-code 0 --severity MEDIUM,HIGH,CRITICAL --ignore-unfixed gossip-app/devsecops:v${BUILD_NUMBER}
                """
            }
        }
    }
