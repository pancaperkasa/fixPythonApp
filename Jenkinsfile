    ...
    stages {
        ...
        stage('Misconfiguration Scanner') {
            steps {
                ...
                trivy config . --format template --template "@html.tpl" -o /var/www/html/trivy/pipeline${BUILD_NUMBER}/report_misconfiguration.html --exit-code 1 --severity MEDIUM,HIGH,CRITICAL
                """
            }
        }
        stage('Build Image') {
        ...
        stage('Vulnerability Image Scanner') {
            steps {
                ...
                trivy image --format template --template "@html.tpl" -o /var/www/html/trivy/pipeline${BUILD_NUMBER}/report_vuln_image.html --exit-code 1 --severity MEDIUM,HIGH,CRITICAL --ignore-unfixed gossip-app/devsecops:v${BUILD_NUMBER}
                """
            }
        }
    }
